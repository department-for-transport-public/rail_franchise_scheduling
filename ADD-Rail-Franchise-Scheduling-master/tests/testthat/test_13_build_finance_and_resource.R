...
# Tests -------------------------------------------------------------------
# Input format is tested in resource and finance tests
...
test_that("Baseline and changed input dataframes are different", {
  ...
  expect_false(all(rfs == rfs_changed))
  expect_false(all(milestone_table == milestone_table_changed))
  #expect_false(all(milestone_fig$x$attrs$`39cc94d694d`$hovertemplate ==
